...
def build_mail_verification_url(token):
    frontend_url = getattr(settings, "FRONTEND_URL", "https://www.ltsertwchanghua.org")
    return f"{frontend_url}/mail-verification/?token={str(token)}"


def build_reset_password_url(uidb64, token):
    frontend_url = getattr(settings, "FRONTEND_URL", "https://www.ltsertwchanghua.org")
    frontend_url = frontend_url.rstrip("/")
    return f"{frontend_url}/login/forgot-password/?uidb64={str(uidb64)}&token={str(token)}"


def send_verification_email(user):
    token = RefreshToken.for_user(user).access_token
    absurl = build_mail_verification_url(token)
    data = {
        "url": absurl,
        "toEmail": user.email,
...
class ResendEmailVerifyAPIView(APIView):
    ...
    def post(self, request):
        ...
        try:
            ...
            if user.is_verified:
                return Response(
                    {"message": "使用者已通過認證"}, status=status.HTTP_409_CONFLICT
                )
            else:
                try:
                    send_verification_email(user)
                    ...
                    response_data = {"message": "驗證信發送失敗，請稍後再試"}
                    if settings.DEBUG:
                        response_data["debug_error"] = str(err)
                    return Response(
...
class UserProfileUpdateAPIView(APIView):
    ...
    def patch(self, request):
        ...
        try:
            ...
            userProfile.has_viewed_verification = data.get(
                "has_viewed_verification", userProfile.has_viewed_verification
            )
    ...
    def post(self, request):
        ...
        if serializer.is_valid():
            ...
            if MyUser.objects.filter(email=email).exists():
                return Response(
                    {"status": "success", "message": "此電子郵件已註冊"},
                    status=status.HTTP_200_OK,
                )
            return Response(
                {"status": "error", "message": "此電子郵件地址未註冊"},
                status=status.HTTP_400_BAD_REQUEST,
            )
        return Response(
            {"status": "error", "message": "無效的數據"},
            status=status.HTTP_400_BAD_REQUEST,
        )
        ...
        if serializer.is_valid():
            ...
            if MyUser.objects.filter(email=email).exists():
                ...
                if user_profile.securityQuestion == security_question:
                    ...
                    absurl = build_reset_password_url(uidb64, token)
                    return Response(
                        {
                            "status": "success",
                            "message": "安全性問題驗證成功",
                ...
                return Response(
                    {"status": "error", "message": "安全性問題錯誤"},
                    status=status.HTTP_400_BAD_REQUEST,
                )
            return Response(
                {"status": "error", "message": "此電子信箱尚未註冊"},
                status=status.HTTP_400_BAD_REQUEST,
            )
        return Response(
            {"status": "error", "message": "無效資料"},
            status=status.HTTP_400_BAD_REQUEST,
        )
...
def export_all_models(request):
    ...
    for model in models:
        ...
        with open(
            f"{temp_dir}/{model_name}.csv", "w", newline="", encoding="utf-8"
        ) as csvfile:
