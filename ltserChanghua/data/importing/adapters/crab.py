...
from data.importing.mappings.crab_mapping import build_crab_payload
from data.models import Crab
...
class CrabAdapter:
    ...
    def __init__(self, hash_fields=None):
        self.hash_fields = hash_fields or list(DEFAULT_CRAB_HASH_FIELDS)
        self.validators = {
    ...
    def build_payload(self, record):
        return build_crab_payload(record, self.validators)
    ...
    def fetch_existing_hash_map(self, keys):
        ...
        return {
            ...
            for data_id, pk, data_hash in Crab.objects.filter(
                dataID__in=keys
    ...
    def write(self, to_create, to_update):
        ...
        with transaction.atomic():
            if to_create:
                Crab.objects.bulk_create(to_create, batch_size=1000)

            if to_update:
                Crab.objects.bulk_update(
                    to_update,
