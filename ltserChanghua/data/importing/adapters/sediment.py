...
from data.importing.mappings.sediment_mapping import build_sediment_payload
from data.models import Sediment
...
class SedimentAdapter:
    key_field = "dataID"

    def __init__(self, hash_fields=None):
        self.hash_fields = hash_fields or list(DEFAULT_SEDIMENT_HASH_FIELDS)
        self.validators = {
    ...
    def build_payload(self, record):
        return build_sediment_payload(record, self.validators)
    ...
    def fetch_existing_hash_map(self, keys):
        ...
        return {
            ...
            for data_id, pk, data_hash in Sediment.objects.filter(
                dataID__in=keys
            ).values_list("dataID", "id", "data_hash")
        }

    def make_instance(self, payload):
        return Sediment(**payload)
    ...
    def write(self, to_create, to_update):
        ...
        with transaction.atomic():
            if to_create:
                Sediment.objects.bulk_create(to_create, batch_size=1000)

            if to_update:
                Sediment.objects.bulk_update(
                    to_update,
